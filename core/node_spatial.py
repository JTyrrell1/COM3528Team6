...
class NodeSpatial(node.Node):
	...
	def jit_init(self, stream_index, img_shape):
		...
		self.state.camera_model_mini = miro.lib.camera_model.CameraModel(self.pars)
	...
	def inject_face(self, stream_index):
		...
		for face in self.state.detect_objects_for_spatial[stream_index].faces:
			...
			if not self.pars.dev.DEBUG_HALT: # do not pollute info
				...
				# flag that indicates a face has been seen
	...
	def publish_best_peak(self):
		...
		# publish that
		self.state.priority_peak = best_peak
	...
	def process_stream(self, stream_index):
		...
		if self.state.in_motion == 0.0:
			...
			if stream_index < 2:
				...
				if self.pars.flags.SALIENCE_FROM_FACE and self.nodes.vision.vision_models_active:
		...
		# publish best peak (augmented with additional information)
		best_peak = self.publish_best_peak()
