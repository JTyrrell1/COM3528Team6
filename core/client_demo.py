...
from node_detect_word import *
from node_detect_music import *
from node_vision import *
from node_blink import *

# Mic imports
import pyaudio

# Choose between "mic" and "miro" - "mic" for the Blue Yeti microphone, "miro" for the MiRo microphone
mic_choice = "miro"
...
class DemoNodes:
	...
	def instantiate(self, sys):
		...
		if self.client_type == "main":
			...
			self.action = NodeAction(sys, mic_choice)
			self.loop = NodeLoop(sys)
			self.spatial = NodeSpatial(sys)
			self.record_audio = NodeRecord(sys)
			self.detect_word = NodeDetectWord(sys, mic_choice)
			#self.detect_mic = NodeDetectWordMic(sys)
			#self.detect_music = NodeDetectMusic(sys)
			self.vision = NodeVision(sys)
			self.blink = NodeBlink(sys)
...
class DemoSystem(object):
	...
	def __init__(self, client_type):
		...
		if self.pars.dev.START_CAMS_HORIZ:
			...
			self.kc_m = miro.lib.kc_interf.kc_miro_cams_horiz()
			self.kc_s = miro.lib.kc_interf.kc_miro_cams_horiz()
		...
		if self.client_type == "main":
			...
			#self.subscribe('sensors/caml/compressed', sensor_msgs.msg.CompressedImage, self.callback_cam)
			#self.subscribe('sensors/camr/compressed', sensor_msgs.msg.CompressedImage, self.callback_cam)
	...
	def audio_stream_callback(self, in_data, frame_count, time_info, status):
		...
		msg = std_msgs.msg.Int16MultiArray(data=audio_data.tolist())
		
		# Publish the message
		self.nodes.detect_word.tick(msg)
	...
	def callback_sensors_package(self, msg):
		...
		if state != self.state_file_contents:
			...
			with open(self.demo_state_filename, 'wt') as file:
				file.write(state)
	...
	def callback_mics(self, msg):
		...
		#print("BEFORE BOTH TICKS: ", rospy.get_time())
		if mic_choice == "miro":
			self.nodes.detect_word.tick(msg)
		
		self.nodes.affect.tick(msg)
		#print("AFTER WORD TICK:", rospy.get_time())
		#self.nodes.detect_music.tick(msg)
		#print("AFTER MUSIC TICK:", rospy.get_time())
