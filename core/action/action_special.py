...
class ActionSpecial(action_types.ActionTemplate):
	...
	def start(self):
		...
		self.vf = miro.lib.camera_model.ViewLine(az, el)
	...
	def audio_init(self):
		...
		# find src file
		file_src = miro.lib.get_media_file("ada_lovelace.mp3")
		if not file_src:
			return
		...
		# normalise wav
		dat = dat.astype(float) # Changed np.float
