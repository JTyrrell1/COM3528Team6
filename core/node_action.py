...
from action.action_dance import ActionDance
from action.action_llm import ActionGPT
from action.action_gestures import ActionGestures
from action.action_test import ActionTest
from action.action_obj_detect import ActionObjectDetect
...
class NodeAction(node.Node):
	...
	def __init__(self, sys, mic_choice):
		...
		#gpt_function = ActionGPT(self) if mic_choice == "miro" else ActionGPTMic(self)
		# create actions
		self.actions = [
				ActionMull(self, mic_choice),
				ActionOrient(self, mic_choice),
				ActionApproach(self, mic_choice),
				ActionFlee(self, mic_choice),
				#ActionAvert(self),
				ActionHalt(self, mic_choice),
				ActionRetreat(self, mic_choice),
				#ActionSpecial(self),
				ActionDance(self, mic_choice),
				#gpt_function,
				ActionGPT(self, mic_choice),
				ActionObjectDetect(self, mic_choice),
				ActionGestures(self, mic_choice)
				#ActionTest(self)
				]
	...
	def modulate_by_cliff(self):
		...
		if self.pars.flags.ACTION_MODULATE_BY_CLIFF:
			...
			if not self.pars.dev.IGNORE_CLIFF_SENSORS:
				...
				cliff -= cliff_min
				cliff = float(np.clip(cliff, 0, cliff_range)) # Changing np.float
	...
	def tick(self):
		...
		# for each action
		#print("PRIORITY FOR EACH ACTION")
		for action in self.actions:
			...
			# Print priority for each action
			#print(str(action.name) + " " + str(action.interface.priority))
